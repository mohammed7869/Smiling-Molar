...
const StepAppointment = ({
  ...
  // Auto-select November 1st if no date is provided
  const getInitialDate = () => {
    if (formData.selectedDate) return formData.selectedDate;
    const currentYear = new Date().getFullYear();
    const novemberFirst = new Date(currentYear, 10, 1); // November 1st (month is 0-indexed)
    novemberFirst.setHours(0, 0, 0, 0);
    return novemberFirst.toISOString().split("T")[0];
  };
  ...
  useEffect(() => {
    ...
    // Auto-select November 1st if no date is selected
    const currentYear = new Date().getFullYear();
    const novemberFirst = new Date(currentYear, 10, 1); // November 1st (month is 0-indexed)
    novemberFirst.setHours(0, 0, 0, 0);
    const novemberFirstDateString = novemberFirst.toISOString().split("T")[0];

    const dateToSelect = formData.selectedDate || novemberFirstDateString;
    ...
    if (dateToSelect) {
      const selectedDateObj = new Date(dateToSelect);
    ...
    // If we auto-selected November 1st, update formData
    if (!formData.selectedDate && dateToSelect === novemberFirstDateString) {
      setFormData({
        ...formData,
        selectedDate: novemberFirstDateString,
      });
    }

    console.log("Local state updated from formData:", {
      newSelectedDate: dateToSelect,
      newSelectedTime: formData.selectedTime || "",
      newSelectedLocationId: formData.selectedLocationId,
      autoSelectedNovemberFirst: !formData.selectedDate,
    });
  }, [
  ...
  const [currentWeekStart, setCurrentWeekStart] = useState(() => {
    ...
    // Set November 1st as the minimum available date and default week
    const currentYear = new Date().getFullYear();
    const novemberFirst = new Date(currentYear, 10, 1); // November 1st (month is 0-indexed)
    novemberFirst.setHours(0, 0, 0, 0);
    const novemberFirstWeekMonday = getMondayOfWeek(novemberFirst);

    // Default to November 1st week
    return novemberFirstWeekMonday;
  });
  ...
  const generateDates = useCallback(() => {
    ...
    // Set November 1st as the minimum available date
    const novemberFirst = new Date(today.getFullYear(), 10, 1); // Month is 0-indexed, so 10 = November
    novemberFirst.setHours(0, 0, 0, 0);
    ...
    for (let i = 0; i < 84; i++) {
      ...
      dates.push({
        ...
        dayKey: dayKey,
        available: loopDate >= novemberFirst, // Changed from today to novemberFirst
      });
    }
  ...
  const navigateWeek = (direction: "prev" | "next") => {
    ...
    // Don't go before November 1st week
    const currentYear = new Date().getFullYear();
    const novemberFirst = new Date(currentYear, 10, 1); // November 1st (month is 0-indexed)
    novemberFirst.setHours(0, 0, 0, 0);
    const novemberFirstWeekMonday = getMondayOfWeek(novemberFirst);
    ...
    const novemberFirstWeekMondayNormalized = new Date(novemberFirstWeekMonday);
    novemberFirstWeekMondayNormalized.setHours(0, 0, 0, 0);

    if (
      direction === "prev" &&
      newDateNormalized < novemberFirstWeekMondayNormalized
    ) {
      // If navigating back would go before November 1st week, don't allow it
      return;
    }
      ...
      <div className="mt-4">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <button
            ...
            disabled={(() => {
              const novemberFirst = new Date();
              novemberFirst.setMonth(10, 1); // November 1st (month is 0-indexed)
              novemberFirst.setHours(0, 0, 0, 0);
              const novemberFirstWeekMonday = getMondayOfWeek(novemberFirst);
              return (
                currentWeekStart.getTime() <= novemberFirstWeekMonday.getTime()
              );
            })()}
          >
